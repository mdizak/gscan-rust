...
fn is_public(ip: &String) -> bool {
    ...
    // Check first segment
    if ip.starts_with("127.") || ip.starts_with("192.") || ip.starts_with("10.") || ip.starts_with("0.") {
        return false;
...
fn check_http(hostname: &str) -> Option<Vec<String>> {
    ...
    // Are we on SSL?
    let mut prefix = "http://";
    if hostname.ends_with(":4443") {
        prefix = "https://";
    }

    // Fire away
    let url = format!("{}{}/", prefix, hostname);
    let res: Response = match HTTP_CLIENT.get(&url).send() {
        Ok(r) => r,
        Err(_) => {
    ...
    let results: Vec<String> = ARGS.search_text.iter().map(|s| {
        s.to_string()
    }).filter(|c| body.contains(c)).collect();
...
fn save_results(hostname: &str, criteria: &String) {
    ...
    let mut fh = match OpenOptions::new()
        ...
        .open(&path) {
            Ok(r) => r,
            Err(e) => panic!("Unable to open results file at {}, error: {}", ARGS.output_file, e)
        };
